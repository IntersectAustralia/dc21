...
attributes :facility_id, :format, :path, :updated_at, :start_time, :end_time, :interval, :file_processing_description, :published, :published_date, :published_by_id, :url, :creator, :access_rights_type,  :if => lambda {|x| x.is_authorised_for_access_by?(current_user)}
attributes :label_names => :labels, :if => lambda {|x| x.is_authorised_for_access_by?(current_user)}
attributes :contributor_names => :contributors, :if => lambda {|x| x.is_authorised_for_access_by?(current_user)}
