...
class OCRWorker
  ...
  def perform
    ...
    begin
      ...
      if [@config.ocr_cloud_host, @config.ocr_cloud_id, @config.ocr_cloud_token].all?(&:present?)
        ...
        def output_response_error(e)
          ...
          if xml_data.elements["error/message"]
            raise "Error: #{xml_data.elements["error/message"].text}"
          else
            raise "Error: #{e.message}. Please contact an administrator."
          end
        end
        ...
        File.open(output_file.path, "w:UTF-8") {|f| f.write(recognized_text.encode('UTF-8', {:invalid => :replace, :undef => :replace})) }
      ...
      output_file.file_processing_description = "This file was automatically generated by OCR (#{ocr_type}). Source file name: #{parent.filename}, source file id: #{output_file.id}"
