...
describe OCRWorker do
  ...
  let(:error_xml) {
    '<?xml version="1.0" encoding="utf-8"?>
      <response>
        <task id="TestOCR_ID" registrationTime="2013-12-20T03:31:59Z" statusChangeTime="2013-12-20T03:32:01Z" status="Error" filesCount="1" credits="0" estimatedProcessingTime="2" />
      </response>'
  }
  let(:transcript) {
    'Home'
  ...
  # check output file processing description of all files
  # check output file contains text from stubbed xml --> /INTERSECT/
  ...
  describe "Error messages" do
    ...
    it "Tesseract should fail if file is of the wrong MIME type" do
      ...
      # mp3 is an audio type supported in Speech Recognition only and should not be supported in OCR
      ...
      output.file_processing_description.should eq "OCR ERROR: Tesseract does not support #{File.join(Rails.root, 'samples/Test_SR.mp3')} (#{parent.format})"
      output.transfer_status.should eq(DataFile::RESQUE_FAILED)
      output.file_processing_status.should eq(DataFile::STATUS_ERROR)
      contents = File.open(output.path, "rb").read
      contents.should be_empty
  ...
  describe "successful conversions" do
    ...
    it "should be successful with ABBY settings" do
      abby_config = {
