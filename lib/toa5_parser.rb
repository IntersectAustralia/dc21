...
class Toa5Parser
  def self.extract_metadata(data_file)
    ...
    if data_line_1
      ...
      data_file.interval = Toa5Utilities.extract_time_from_data_line(data_line_2, delimiter) - start_time if (data_line_2 && start_time)
    end
    data_file.end_time = Toa5Utilities.extract_time_from_data_line(last_line, delimiter) if last_line
